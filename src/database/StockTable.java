...
public class StockTable{
	...
	public void insert(Stocks p){
		try{
			...
			stmt.executeUpdate("INSERT INTO stocks VALUES('"+p.getProduct().getId()+
											"','"+p.getUser().getId()+"','"+p.getTanggal()+
											"','"+p.getWaktu()+"','"+p.getJumlah()+"','"+p.getStok()+"')");
			stmt.executeUpdate("UPDATE products SET stok="+p.getStok()+") "
					+ "WHERE id='"+p.getProduct().getId()+"'");
			stmt.close();
	...
	public Vector<Stocks> cariByIdB(String idB){
		...
		try{
			Statement stmt=koneksi.createStatement();
			ResultSet rs=stmt.executeQuery("SELECT * FROM stocks WHERE id_barang='"+idB+"'");
			while(rs.next()){
				String idU=rs.getString(2);
				String tanggal=rs.getString(3);
				String waktu=rs.getString(4);
	...
	public Vector<Stocks> cariByIdU(String idU){
		...
		try{
			Statement stmt=koneksi.createStatement();
			ResultSet rs=stmt.executeQuery("SELECT * FROM stocks WHERE id_user='"+idU+"'");
			while(rs.next()){
				String idB=rs.getString(1);
				String tanggal=rs.getString(3);
				String waktu=rs.getString(4);
	...
	public Vector<Stocks> cariByIdTanggal(String tanggal){
		...
		try{
			Statement stmt=koneksi.createStatement();
			ResultSet rs=stmt.executeQuery("SELECT * FROM stocks WHERE tanggal='"+tanggal+"'");
			while(rs.next()){
				String idB=rs.getString(1);
				String idU=rs.getString(2);
				String waktu=rs.getString(4);
	...
	public Stocks getStockDetil(String idB){
		...
		try{
			Statement stmt=koneksi.createStatement();
			String maxTgl="(select max(tanggal) FROM stocks WHERE id_barang='"+idB+"')";
			String maxWkt="(select max(waktu) FROM stocks WHERE id_barang='"+idB+"' and tanggal="+maxTgl+")";
			ResultSet rs=stmt.executeQuery("SELECT * FROM stocks WHERE id_barang='"+idB+"' and "
